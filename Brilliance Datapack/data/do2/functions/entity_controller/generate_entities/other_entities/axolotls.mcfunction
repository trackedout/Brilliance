...
scoreboard players set $dungeon do2.tests.amountOfMobsExists 0
execute as @e[type=axolotl,tag=AXO_2] run scoreboard players add $dungeon do2.tests.amountOfMobsExists 1
execute unless score $dungeon do2.tests.amountOfMobsExists matches 1 run kill @e[type=axolotl,tag=AXO_2]
execute unless score $dungeon do2.tests.amountOfMobsExists matches 1 run summon minecraft:axolotl -512 11 1992 {Tags: ["AXO_2","do2.manually_spawned"], CustomName: '{"text":"Linda Flynn-Fletcher"}', Fire: -1s, Invulnerable: 1b, PersistenceRequired: 1b, FromBucket: 1b, Health: 14.0f, Variant: 0}

# Perry
scoreboard players set $dungeon do2.tests.amountOfMobsExists 0
execute as @e[type=axolotl,tag=AXO_3] run scoreboard players add $dungeon do2.tests.amountOfMobsExists 1
execute unless score $dungeon do2.tests.amountOfMobsExists matches 1 run kill @e[type=axolotl,tag=AXO_3]
execute unless score $dungeon do2.tests.amountOfMobsExists matches 1 run summon minecraft:axolotl -509 11 1992 {Tags: ["AXO_3","do2.manually_spawned"], CustomName: '{"text":"Perry"}', Fire: -1s, Invulnerable: 1b, PersistenceRequired: 1b, FromBucket: 1b, Health: 14.0f, Variant: 1}
