...
# Setup
execute store result score $dungeon do2.utility.ec.mobNamesCount run data get storage do2:mobs level1_zone3
scoreboard players operation $dungeon do2.utility.ec.mobGenerationCount = $l1z3 do2.config.ec.levelZoneMobCount
...
execute if score $l1z3 do2.config.ec.levelZoneMobCount matches ..0 if score $dungeon do2.utility.ec.collectingLogs matches 1 run data modify storage do2:mobs mobCountErrors append value 'L1Z3'
execute if score $l1z3 do2.config.ec.levelZoneMobCount matches ..0 if score $dungeon do2.utility.ec.collectingLogs matches 0 as @a[scores={do2.logs.dungeon_setup=3..}] run tellraw @s ["",{"text":"[§9B§r]: L1Z3's mob count is empty. No mobs will generate in this zone."}]
execute if score $l1z3 do2.config.ec.levelZoneMobCount matches ..0 run scoreboard players set $l1z3 do2.config.ec.levelZoneMobCount 0
...
execute if score $dungeon do2.utility.ec.mobNamesCount < $l1z3 do2.config.ec.levelZoneMobCount run function do2:entity_controller/config_interface/add_mob_names/add_name_to_list
...
# Agronet event
function do2:external/agronet/logs/entity_controller/areas/l1z3