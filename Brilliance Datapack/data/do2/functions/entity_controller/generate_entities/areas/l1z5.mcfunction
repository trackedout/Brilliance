# - Start Log -
execute as @a[scores={do2.logs.entity_controller=2..}] run tellraw @s ["",{"text":"[§9B§r]: Creating all §bL1Z5§r entities."}]
# - End Log -

# Setup
execute store result score $dungeon do2.utility.ec.mobNamesCount run data get storage do2:mobs level1_zone5
...
execute if score $l1z5 do2.config.ec.levelZoneMobCount matches ..0 if score $dungeon do2.utility.ec.collectingLogs matches 1 run data modify storage do2:mobs mobCountErrors append value 'L1Z5'
execute if score $l1z5 do2.config.ec.levelZoneMobCount matches ..0 if score $dungeon do2.utility.ec.collectingLogs matches 0 as @a[scores={do2.logs.dungeon_setup=3..}] run tellraw @s ["",{"text":"[§9B§r]: L1Z5's mob count is empty. No mobs will generate in this zone."}]
execute if score $l1z5 do2.config.ec.levelZoneMobCount matches ..0 run scoreboard players set $l1z5 do2.config.ec.levelZoneMobCount 0
...
execute if score $dungeon do2.utility.ec.mobNamesCount < $l1z5 do2.config.ec.levelZoneMobCount run function do2:entity_controller/config_interface/add_mob_names/add_name_to_list
...
execute if score $dungeon do2.config.fr.teleportKillers matches 1 as @e[tag=premade_generated_mob] run tp @s @e[type=marker,tag=L1Z5,limit=1,sort=random]
execute as @e[tag=premade_generated_mob] run tag @s add L1Z5
execute as @e[tag=premade_generated_mob] run tag @s remove premade_generated_mob

# Agronet event
function do2:external/agronet/logs/entity_controller/areas/l1z5