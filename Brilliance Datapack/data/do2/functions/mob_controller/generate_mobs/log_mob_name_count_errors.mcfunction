...
execute if score $dungeon do2.utility.mc.collectingLogs matches ..0 run return 0
execute if score $dungeon do2.utility.mc.collectingLogs matches 1 as @a[scores={do2.logs.dungeon_setup=3..}] run tellraw @s ["",{"text":"[§9B§r]: "},{"storage": "do2:mobs","nbt": "mobNameCountErrors[0]"},{"text":"'s mob name count is empty. Mobs' names will be randomly generated in this zone."}]
execute if score $dungeon do2.utility.mc.collectingLogs matches 1 run return 0
