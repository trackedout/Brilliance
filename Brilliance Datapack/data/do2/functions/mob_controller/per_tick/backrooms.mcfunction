execute if block -528 117 1943 minecraft:lever[powered=true] as @e[type=marker,tag=L2WILLIE] unless entity @s[tag=frustrated_visible] at @s run function do2:dev/markers/turn_visible/l2willie
execute if block -528 117 1943 minecraft:lever[powered=false] if entity @e[tag=frustrated_visible,tag=L2WILLIE] run function do2:dev/markers/turn_invisible/l2willie
