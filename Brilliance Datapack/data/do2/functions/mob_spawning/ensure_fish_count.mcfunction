...
execute if score $dungeon do2.config.amountOfFish > $dungeon do2.utility.currentFishCount at @r at @e[distance=24..64,type=marker,tag=fish_marker,limit=1,sort=random] unless entity @e[type=tropical_fish,distance=..3] function do2:mob_spawning/fish_counts/summon_fish
