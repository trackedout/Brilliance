...
execute if score $dungeon do2.config.amountOfFish < $dungeon do2.utility.currentFishCount at @e[type=marker,tag=fish_marker,limit=1,sort=random] run function do2:mob_spawning/fish_counts/summon_fish
