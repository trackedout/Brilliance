...
execute as @a[scores={do2.logs.pickups=1..}] if score @p[tag=berriesPickUpLogTarget] do2.utility.berriesToGive matches 1 run tellraw @s ["",{"text":"[§9B§r]: "},{"selector":"@p[tag=berriesPickUpLogTarget]"},{"text":" picked up §b"},{"color":"aqua","score":{"name":"@p[tag=berriesPickUpLogTarget]","objective":"do2.utility.berriesToGive"}},{"text":" sweet berry."}]
