...
execute as @a[scores={do2.logs.pickups=2..}] if score @p[tag=pumpkinPickUpLogTarget] do2.utility.pumpkinsToGive matches 1 run tellraw @s ["",{"text":"§f[§9B§r]: "},{"selector":"@p[tag=pumpkinPickUpLogTarget]"},{"text":" picked up §b"},{"color":"aqua","score":{"name":"@p[tag=pumpkinPickUpLogTarget]","objective":"do2.utility.pumpkinsToGive"}},{"text":" halloween pumpkin."}]
execute as @a[scores={do2.logs.pickups=2..}] if score @p[tag=pumpkinPickUpLogTarget] do2.utility.pumpkinsToGive matches 2.. run tellraw @s ["",{"text":"§f[§9B§r]: "},{"selector":"@p[tag=pumpkinPickUpLogTarget]"},{"text":" picked up §b"},{"color":"aqua","score":{"name":"@p[tag=pumpkinPickUpLogTarget]","objective":"do2.utility.pumpkinsToGive"}},{"text":" halloween pumpkins."}]
tag @s remove pumpkinPickUpLogTarget
# - End Log -

# Agronet event
function do2:external/agronet/logs/pickups/halloween_pumpkins
