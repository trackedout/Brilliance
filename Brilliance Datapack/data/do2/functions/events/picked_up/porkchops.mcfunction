...
execute as @a[scores={do2.logs.pickups=1..}] unless score @p[tag=chopsPickUpLogTarget] do2.utility.chopsToGive matches 1 run tellraw @s ["",{"text":"§f[§9B§r]: "},{"selector":"@p[tag=chopsPickUpLogTarget]"},{"text":" picked up §b"},{"color":"aqua","score":{"name":"@p[tag=chopsPickUpLogTarget]","objective":"do2.utility.chopsToGive"}},{"text":" porkchops."}]
