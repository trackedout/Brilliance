...
execute as @a[scores={do2.logs.pickups=1..}] run tellraw @s ["",{"text":"[§9B§r]: "},{"selector":"@p[tag=spsPickUpLogTarget]"},{"text":" picked up an artifact. ("},{"text":" ? ","color":"dark_red","hoverEvent":{"action":"show_text","contents":["",{"text":"§fID: §9#44\n§fValue: §b48\n§fName: §aStaff of the Pink Shepherd\n§fLore:\n§o§5Honestly... Nobody really knows\nwhat this thing does."}]}},{"text":")"}]
...
scoreboard players set $dungeon do2.run.foundArtifact 44
scoreboard players add $dungeon do2.artifacts.SPS 1
scoreboard players set $dungeon do2.run.artifactValue 48

# Agronet event
function do2:external/agronet/logs/pickups/staff_of_the_pink_shepherd