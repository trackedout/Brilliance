...
execute as @a[scores={do2.logs.pickups=1..}] unless score @p[tag=key_2PickUpLogTarget] do2.utility.key_2sToGive matches 1 run tellraw @s ["",{"text":"§f[§9B§r]: "},{"selector":"@p[tag=key_2PickUpLogTarget]"},{"text":" picked up §b"},{"color":"aqua","score":{"name":"@p[tag=key_2PickUpLogTarget]","objective":"do2.utility.key_2sToGive"}},{"text":" level 2 keys."}]
