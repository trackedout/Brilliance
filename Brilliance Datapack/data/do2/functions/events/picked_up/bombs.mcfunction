...
execute as @a[scores={do2.logs.pickups=1..}] unless score @p[tag=bombPickUpLogTarget] do2.utility.bombsToGive matches 1 run tellraw @s ["",{"text":"§f[§9B§r]: "},{"selector":"@p[tag=bombPickUpLogTarget]"},{"text":" picked up §b"},{"color":"aqua","score":{"name":"@p[tag=bombPickUpLogTarget]","objective":"do2.utility.bombsToGive"}},{"text":" bombs."}]
