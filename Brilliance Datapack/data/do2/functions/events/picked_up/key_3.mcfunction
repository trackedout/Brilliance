...
execute as @a[scores={do2.logs.pickups=1..}] unless score @p[tag=key_3PickUpLogTarget] do2.utility.key_3sToGive matches 1 run tellraw @s ["",{"text":"§f[§9B§r]: "},{"selector":"@p[tag=key_3PickUpLogTarget]"},{"text":" picked up §b"},{"color":"aqua","score":{"name":"@p[tag=key_3PickUpLogTarget]","objective":"do2.utility.key_3sToGive"}},{"text":" level 3 keys."}]
