...
execute if score $dungeon do2.utility.currentTick matches 0 run function do2:events/on_datapack_tick

# - Start Log -
execute as @a[scores={do2.utility.logLevel=3..}] run tellraw @s ["",{"text":"[§9B§r]: Game ticked. ("},{"score":{"name":"$dungeon","objective":"do2.utility.checkTick"},"color":"aqua"},{"text":"§r/"},{"score":{"name":"$dungeon","objective":"do2.config.tickRate"},"color":"aqua"},{"text":"§r)"}]
# - End Log -
