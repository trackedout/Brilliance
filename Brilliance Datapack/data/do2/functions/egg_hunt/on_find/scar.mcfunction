...
execute if score $dungeon do2.config.eggRewards matches 1 unless score @s do2.eggs.scar matches 2 run playsound minecraft:block.barrel.open master @s -606 4 1884
execute if score $dungeon do2.config.eggRewards matches 1 unless score @s do2.eggs.scar matches 2 run data modify block -606 4 1884 Items set value [{Slot: 13b, id: "minecraft:iron_nugget", Count: 15b, tag: { CustomModelData: 2, tracked:1b, display: {Name: '{"text":"❄☠ Decked Out Crown ☠❄"}'}}}]
execute if score $dungeon do2.config.eggRewards matches 1 unless score @s do2.eggs.scar matches 2 run scoreboard players set @s do2.eggs.scar 2
