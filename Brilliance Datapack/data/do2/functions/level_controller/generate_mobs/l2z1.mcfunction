...
execute if score $l2z1 do2.config.mc.levelZoneMobCount matches ..0 as @a[scores={do2.logs.dungeon_setup=3..}] run tellraw @s ["",{"text":"[§9B§r]: L2Z1's mob count is empty. No mobs will generate in this zone."}]
...
execute if score $dungeon do2.utility.mobNamesCount < $l2z1 do2.config.mc.levelZoneMobCount as @a[scores={do2.logs.dungeon_setup=3..}] run tellraw @s ["",{"text":"[§9B§r]: Not enough L2Z1's mob names for L2Z1's mob count. Adding random names to fix."}]
execute if score $dungeon do2.utility.mobNamesCount < $l2z1 do2.config.mc.levelZoneMobCount run function do2:level_controller/add_mob_names/ravager_names
