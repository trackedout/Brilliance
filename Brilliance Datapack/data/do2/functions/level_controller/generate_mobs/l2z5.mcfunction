...
execute if score $dungeon do2.utility.mobNamesCount < $l2z5 do2.config.mc.levelZoneMobCount as @a[scores={do2.logs.dungeon_setup=3..}] run tellraw @s ["",{"text":"[§9B§r]: Not enough L2Z5's mob names for L2Z5's mob count. Adding random names to fix."}]
execute if score $dungeon do2.utility.mobNamesCount < $l2z5 do2.config.mc.levelZoneMobCount run function do2:level_controller/add_mob_names/ravager_names
