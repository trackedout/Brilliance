...
execute unless score $dungeon do2.tests.all_wardens_alive matches 0 as @a[scores={do2.logs.dungeon_setup=3..}] run tellraw @s ["",{"text":"[§9B§r]: Missing Warden §b#"},{"color":"light_blue","score":{"name":"$dungeon","objective":"do2.tests.all_wardens_alive"}}]
