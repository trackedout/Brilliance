...
tellraw @s ["",{"clickEvent":{"action":"run_command","value":"/trigger do2.trigger.vanilla set 31"},"translate":"do2.buttons.enable","fallback":"%s","with":[{"text":"[ENABLE]","color":"green"}]}," ",{"clickEvent":{"action":"run_command","value":"/trigger do2.trigger.vanilla set 30"},"translate":"do2.buttons.darker_disable","fallback":"%s","with":[{"text":"[DISABLE]","color":"dark_red"}]}]
tellraw @s ["","\n",{"text":"-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-","color":"gold"}]
