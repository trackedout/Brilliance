...
Brilliance uses a ton of scoreboard objectives for a vast amount of things. This list should compile it all into it's many sections.
1. Data Tracking
   - These are scoreboard values that DO get shared. These are the most important to *Agronet* as these values need to be passed back and forth between each run
2. Utility
   - These are scoreboard values that DOESN'T get shared, and is only used by Brilliance to set up other scores.
   - Example: `do2.utility.reachedBottomDepthCharge` is to check if the player has reached the bottom of the tnt depth charge. We don't care if they reached the top UNLESS they have reached the bottom first.
...
2. @s
   ...
   3. ⛔ | The score is stored on `@a[tag=shulker_recieved]`
3. Name
...
|  ❌   |  ❌  | do2.run.useZones                  | Current  | Boolean on whether the run is using Ravager Zones.                                                         |
